...
먼저 헷갈릴 수 있는 개념인 서버 컴포넌트와 서버사이드 렌더링에 대해 짚고 넘어가자. 서버 컴포넌트 서버사이드 렌더링과는 근본적으로 다르다. 서버 컴포넌트는 리액트 돔이 클라이언트가 아닌 서버에서 실행된다는 개념이다.\
보통의 경우 웹페이지를 서버사이드에서 렌더링하면 클라이언트는 아래의 과정을 거친다
1. 리액트 코드를 다운로드한다. 이 리액트 코드는 하이드레이팅을 수행하며 그 외의 여러가지 리액트 로직을 담고있다
1. Html코드를 하이드레이팅 하여 리액트가 사용할 수 있는 형태로 만든다
위의 과정은 두가지 관점에서 속도를 저하시킨다. 첫쨰로 리액트 코드를 다운로드하여 파싱하는 과정인데 이 과정에서 시간이 소모된다. 그리고 html코드를 하이드레이팅 하는 과정에서 렌더러 스레드가 자바스크립트 실행 엔진에 의해 장시간 선점된다. 렌더러 스레드가 선점되는 동안은 페이지가 프리징되며 UX를 저하시킨다.\
만일 이 두가지 과정 중 하나라도 건너뛸 수 있다면 최초의 페이지 로딩 속도를 크게 개선할 수 있다. 그렇게 해서 고안된 개념이 서버 컴포넌트이다.\
앞서 말했듯 서버 컴포넌트는 리액트 돔이 클라이언트가 아닌 서버에서 실행된다. 따라서 클라이언트에서 내려받는 리액트 코드는 크게 감소한다. 그렇게 내려받은 리액트 코드를 파싱하여 실행하는데 파일의 크기가 크게 줄어들었으므로 파싱하는 과정에서 속도가 크게 개선된다.\
\ 서버 컴포넌트를 사용하면 클라이언트 사이드의 자바스크립트 코드는 대부분 필요가 없어진다. 그 결과로 자바스크립트를 파싱할 필요가 없으므로 초기 페이지 렌더링 속도를 크게 줄일 수 있다. 이는 UX 증진에 큰 영향을 미친다.
server-side rendering and server-component is actually complementary and you can use both together

서버 컴포넌트는 컴포넌트를 생성하는데 필요한 자바스크립트 파일을 다운로드할 필요가 없다. 즉 서버사이드에서 생성된 정적 html파일을 넘겨받을 뿐이다. 다시말해 자바스크립트를 이용한 저와의 상호작용이 불가능한데 만일 유저와의 상호작용이 있는 컴포넌트는 서버 컴포넌트를 사용할 수 없다
